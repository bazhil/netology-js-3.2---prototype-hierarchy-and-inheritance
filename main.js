...
function registerNewUser(data) {
	if (!isValidEmail(data.email)) {
		throw new NotValidEmailRegistrationError(
...
class SpaceDate extends Date {
  ...
  constructor (...args) {
	...
	getNextDate() {
    return new SpaceDate(this.getFullYear(), this.getMonth(),this.getDate() + 1)
	}
	...
	getDayBeginning() {
    return new SpaceDate(this.getFullYear(), this.getMonth(),this.getDate(), 0 , 0, 0, 0)
	}

	getDayEnd() {
        return new SpaceDate(this.getFullYear(), this.getMonth(),this.getDate(), 23 , 59, 59, 999)
	}
}
...
console.log(
	`Дата заявки поставщику: ${requestDate.toLocaleDateString('ru-Ru')}`
);
...
console.log(
	`В любое время с ${from.toLocaleString('ru-Ru')} по ${to.toLocaleString(
		'ru-Ru'
	)}`
);
...
class AllDayExceptHolidaysPaymentTerminal extends PaymentTerminal {
	...
	checkActive(calendar) {
		return JSON.stringify(holidays[0]) !== JSON.stringify({date: this.calendar.now.getDate(), month: this.calendar.now.getMonth()}) & JSON.stringify(holidays[1]) !== JSON.stringify({date: this.calendar.now.getDate(), month: this.calendar.now.getMonth()})
	}
}
...
class WorkspacePaymentTerminal extends PaymentTerminal {
	...
	checkActive(calendar) {
    return this.calendar.now.getHours() >= 8 && this.calendar.now.getHours() < 18
	}
}

const holidays = [{ date: 11, month: 3 - 1 }, { date: 23, month: 2 - 1 }];
...
const terminals = [
	...
	new AllDayExceptHolidaysPaymentTerminal(
		'Терминал в торговом центре',
		calendar,
		holidays
	),
];
